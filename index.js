...
embedded_app.on('application:initializeError', (payload) => {
  const errorName = webex.Application.ErrorCodes[payload.code] || 'UNKNOWN';
  log('application:initializeError', {code: payload.code, name: errorName}, 'event');
});
...
const sidebarButtons = {
  ...
  notificationRedirectUrl: document.getElementById("notification-redirect-url"),
};
...
embedded_app.onReady().then(() => {
  ...
  embedded_app.listen().then(() => {
    ...
    embedded_app.on("meeting:createInteraction", (payload) =>
      log("meeting:createInteraction", payload, "event")
    );
    ...
    embedded_app.on("application:tokenRefreshed", (payload) => {
      log("application:tokenRefreshed", payload, "event");
    });
  });
});
...
function handleShowNotification() {
  if (!sidebar) {
    log(
      "showNotification() promise failed as sidebar info is not available",
      webex.Application.ErrorCodes[4]
    );
    return;
  }

  const message = sidebarButtons.notificationMessage.value;
  const redirectURL = sidebarButtons.notificationRedirectUrl.value;

  sidebar
    .showNotification(message, redirectURL)
    .then((success) => {
      log("showNotification()", { message, redirectURL });
    })
    .catch((error) => {
      log(
        "showNotification() promise failed with error",
        webex.Application.ErrorCodes[error]
      );
    });
}
